...
\vspace{1em}
\newline
To back propagate through the network, we need to find how much of the error (cost $C$) is due to each node in the network.
...
\begin{align}
    \frac{\delta C}{\delta \alpha^{\mathcal{L}}_m} = \sum_{p=1}^{q}\Big(\frac{\delta C}{\delta \omega^{\mathcal{L}+1}_{mp}}\Big) = \delta^{\mathcal{L}+1}_{q}
\end{align}
What we just derived is the delta value of a node. Its the cumulative error of that node, and the error of the nodes it is connected to, propagated back through the network.
Finally substituting this back into (2.6.7) and (2.6.8) we get the following:
\begin{align}
    \frac{\delta C}{\delta \omega^{\mathcal{L}}_{nm}} &= \alpha_{n}^{\mathcal{L}-1} \cdot \mathcal{F}'_{\mathcal{L}}(\mathcal{S}^{\mathcal{L}}_m) \cdot \delta^{\mathcal{L}+1}_{q}\\
    \frac{\delta C}{\delta \beta^{\mathcal{L}}_{m}} &= \mathcal{F}'_{\mathcal{L}}(\mathcal{S}^{\mathcal{L}}_m)\cdot\delta^{\mathcal{L}+1}_{q}
\end{align}
\vspace{1em}
\newline
For the output layer, the delta value is the output of the cost function, as 100\% of the error comes through the output layer.
\begin{equation}
    \delta^{m}_{n} = C(E_{n},O_{n})
\end{equation}
Where $C(C_{n},\mathcal{O}_{n})$ is the loss function, $E_{n}$ is the expected output and $O_{n}$ is the predicted output of the node.
\vspace{1em}
\newline
We can then construct an equation for the delta values of the hidden nodes in the network.

\textit{For the hidden nodes in layers $1\leq k < m$:}
\begin{equation}
    \delta^{k}_{n} = \sum_{q=1}^{p}\Big(\omega^{k+1}_{nq}\cdot\delta^{k+1}_{q}\Big)
\end{equation}
...
\begin{equation}
    \omega'^{k}_{nq} = \omega^{k}_{nq} + \eta\cdot\delta^{k}_{q}\cdot\alpha_{n}^{k-1} \cdot\mathcal{F}'_{m}(\mathcal{S}^{k}_{q})
\end{equation}

\begin{equation}
    \beta'^{k}_{nq} = \beta^{k}_{nq} + \eta\cdot\delta^{k}_{q} \cdot\mathcal{F}'_{m}(\mathcal{S}^{k}_{q})
\end{equation}
